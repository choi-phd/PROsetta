...
The frequency distribution of the summed scores for the combined scale can be plotted as a histogram with `plot()`. The required argument is a "PROsetta" data object created with `loadData()`. The optional `scale` argument specifies for which scale the summed score should be created. Setting `scale = 'combined'` plots the summed score distribution for the combined scale.
...
Basic descriptive statistics are obtained for each item by `runDescriptive()`. The statistics include `n`, `mean`, `sd`, `median`, `min`, `max`, `skew`, and `kurtosis`.
...
Classical reliability statistics can be obtained by `runClassical()`. By default, the analysis is performed for the combined scale. The user can set `scalewise = TRUE` to request an analysis for each scale separately in addition to the combined scale.
...
`runCalibration()` performs IRT calibration without anchoring.  `runCalibration()` calls `mirt::mirt()` internally. 
...
In case of nonconvergence, `runCalibration()` explicitly raises an error and does not return its results. Additional arguments can be passed onto `mirt`, e.g., to increase the number of EM cycles to 1000, as follows:
...
* `escore_3`: expected summed score (true score) for the combined scale given the EAP estimate
...
* `type`: the type of equating to be performed, `equipercentile` for this example. See `?equate::equate` for details.
...
```{r}
plot(rsss_fixedpar$`2`[["raw_2"]], rsss_fixedpar$`2`[["t_score"]], xlab = "CES-D Summed Score", ylab = "PROMIS Depression T-score", type = "l", col = "blue")
lines(out_equate_tscore$concordance[["raw_2"]], out_equate_tscore$concordance[["equiv_1"]], lty = 2, col = "red")
grid()
...
To begin with, we create a variable, `sum_score_cesd`, using `getScaleSum()` to contain raw summed scores on Scale 2 (i.e., CES-D). `NA` will result for any respondents with one or more missing responses on Scale 2. We could also create a summed score variable for Scale 1 using the same function, e.g., `getScaleSum(d, 1)`.
...
```{r}
tscore_promis = data.frame(prosettaid = eap_promis$prosettaid, tscore_promis = round(eap_promis$theta_eap * 10 + 50, 1))
head(tscore_promis)
```
...
```{r}
scores = scores %>% inner_join(tscore_promis, by = "prosettaid")
head(scores)
```
...
First, we get EAP estimates based on item response patterns on Scale 2 using the CES-D item parameters linked to the PROMIS Depression metric (via fixed-parameter calibration). We then linearly transform the EAP estimates to T-scores and add the T-scores (`tscore_cesd_pattern`) to the data frame object `scores`.

```{r, message = FALSE}
eap_cesd            = getTheta(d, out_link_fixedpar$ipar_linked, scale = 2)$theta
tscore_cesd_pattern = data.frame(prosettaid = eap_cesd$prosettaid, tscore_cesd_pattern = round(eap_cesd$theta_eap * 10 + 50, 1))
scores = scores %>% inner_join(tscore_cesd_pattern, by = "prosettaid")
head(scores)
```
...
Second, we use the raw-score-to-scale-score (RSSS) crosswalk table obtained above using summed score EAP estimation to map each raw summed score on Scale 2 onto a T-score on the PROMIS Depression metric, `tscore_cesd_rsss`. 

```{r, message = FALSE}
rsss_eap = data.frame(raw_2 = rsss_fixedpar$`2`[["raw_2"]], tscore_cesd_rsss = round(rsss_fixedpar$`2`[["t_score"]], 1))
...
Third, we use the concordance table from equipercentile linking to map each raw summed score on Scale 2 onto a T-score on the PROMIS Depression metric, `tscore_cesd_eqp`.

```{r, message = FALSE}
rsss_eqp = data.frame(raw_2 = out_equate_tscore$concordance[["raw_2"]], tscore_cesd_eqp = round(out_equate_tscore$concordance[["equiv_1"]], 1))
...
stats_pattern   = compareScores(scores$tscore_promis, scores$tscore_cesd_pattern) ## IRT response pattern EAP to T-score
stats_rsss      = compareScores(scores$tscore_promis, scores$tscore_cesd_rsss)    ## IRT summed score EAP to T-score
stats_eqp       = compareScores(scores$tscore_promis, scores$tscore_cesd_eqp)     ## Equipercentile summed score to T-score

stats           = rbind(stats_pattern, stats_rsss, stats_eqp)
